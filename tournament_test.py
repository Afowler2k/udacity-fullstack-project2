...
def testPairingsNoRematch():
    deleteMatches()
    deletePlayers()
    registerPlayer("Twilight Sparkle")      #1
    registerPlayer("Fluttershy")            #2
    registerPlayer("Applejack")             #3
    registerPlayer("Pinkie Pie")            #4
    registerPlayer("Bruno Walton")          #5
    registerPlayer("Boots O'Neal")          #6
    registerPlayer("Cathy Burton")          #7
    registerPlayer("Diane Grant")           #8
    standings = playerStandings()
    ...
    if correct_pairs != actual_pairs:
        raise ValueError(
            "After two matches, players not paired correctly.")
    #round 3
    reportMatch(pid1, pid2)
    reportMatch(pid3, pid4)
    reportMatch(pid5, pid6)
    ...
    if correct_pairs != actual_pairs:
        raise ValueError(
            "After three matches, players not paired correctly.")
    #round 4
    reportMatch(pid1, pid2)
    reportMatch(pid3, pid4)
    reportMatch(pid5, pid6)
